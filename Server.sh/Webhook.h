...
void send_PlayerLogin(string name, string ip) {
	...
	dpp::embed embed = dpp::embed().
		...
		set_footer(dpp::embed_footer().set_text("©Growhoshi").set_icon("https://cdn.growtopia.tech/items/18.png"));
	bot.execute_webhook_sync(wh, dpp::message().add_embed(embed));
}
void send_PlayerReg(string name, string email, string ip) {
	srand(time(0));
	dpp::cluster bot("");
	bot.on_log(dpp::utility::cout_logger());
	dpp::webhook wh("https://discord.com/api/webhooks/1056023496490233916/fvVZbpqt_1MOIDuRf7oKh1QbzJ9QZiJQ8v8jxo5_f0Sg5fy8EGhV_Rp0a3jTrIvLkTL-");
	dpp::embed embed = dpp::embed().
		set_color(dpp::colors::summer_sky).
		set_author("Player Register", "", "https://cdn.growtopia.tech/items/32.png").
		add_field(
			"Username",
			name
		).
...
void sendWebhook(string dqitems, string eventday) {
	...
	dpp::embed embed = dpp::embed().
		...
		set_footer(dpp::embed_footer().set_text("Item price is subject to change based on demand | Generated by: ©Growhoshi ~ Ritshu#0228 | (☎ 12345)").set_icon("https://cdn.growtopia.tech/items/3902.png"));
	bot.execute_webhook_sync(wh, dpp::message().add_embed(embed));
}
void send_Sb(string player, string text, string world) {
	...
	dpp::embed embed = dpp::embed().
		...
		set_footer(dpp::embed_footer().set_text("©Growhoshi").set_icon("https://cdn.growtopia.tech/items/2480.png"));
	bot.execute_webhook_sync(wh, dpp::message().add_embed(embed));
}
...
void send_Trade(string name, string tradeitem1, string name2, string tradeitem2) {
	...
	dpp::embed embed = dpp::embed().
		...
		set_footer(dpp::embed_footer().set_text("©Growhoshi").set_icon("https://cdn.growtopia.tech/items/3902.png"));
	bot.execute_webhook_sync(wh, dpp::message().add_embed(embed));
}
